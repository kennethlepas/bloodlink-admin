document.addEventListener('DOMContentLoaded', () => {
    ...
    if (hospital || (user && user.role === 'hospital_admin')) {
        ...
        console.log('Filtering notifications for hospital:', hospitalId);
        // Notifications directed to this specific hospital
        query = query.where('recipientId', '==', hospitalId);
    ...
    window.handleNotificationClick = (id, type, data) => {
        markAsRead(id);
        const prefix = (user && user.role === 'hospital_admin') ? '/hospital' : '';
        ...
        if (type.includes('verify')) {
            ...
            const targetId = data.senderId || data.userId;
            const targetName = data.senderName || 'Contact';
